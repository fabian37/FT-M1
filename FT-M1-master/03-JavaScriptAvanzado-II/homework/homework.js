...
function cacheFunction(cb) {
  let cache = {}
  return function (arg) {
    //IF = si la propiedad no está, lo voy a calcular por primera vez, almacenarlo en la "cache" y retornarlo
    //ELSE = si el resultado de la operacion ya esta en la memoria en la "cache", lo retorno de ahí directamente
    if (!cache.hasOwnProperty(arg)) return cache[arg]=cb(arg)      
    return cache[arg]
  }
}
